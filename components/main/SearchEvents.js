...
export const SearchEvents = (props) => {
  ...
  const Tag = (props) => {
    ...
    const color =
      selectedTags.indexOf(tagName) === -1 ? "lightgray" : "#5db075";
    ...
    <View style={styles.container}>
      <View style={{ display: showSearch }}>
        <FancyInput
          ...
          onChangeText={(search) => {
            ...
            setTempSearch(searchLower);
            setIsLoading(true);
          }}
          onSubmitEditing={() => userSearch(tempSearch)}
      ...
      <FlatList
        ...
        renderItem={(event) => (
          ...
          >
            <Card key={event.item.ID} id={event.item.ID} loading={isLoading} />
          </TouchableOpacity>
        )}
        style={{ margin: 15 }}
